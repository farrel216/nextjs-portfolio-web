...
const Portfolio = ({ title, img, alt, tech, github, demo, className }) => {
    // console.log(tech)
    const [showGit, setShowGit] = React.useState(false)
    const [showDemo, setShowDemo] = React.useState(false)
    return (
        <>
            <div className="group relative overflow-hidden border-2 rounded-xl min-w-[500px] h-64">
                ...
                <Image className="group-hover:scale-125 transition-all duration-500 w-full h-full" src={img ? img : placehold} alt={alt} />
                ...
                <div className="absolute left-12 -bottom-full group-hover:bottom-16 transition-all duration-700 z-50">
                    <span className="text-3xl text-light">{title}</span>
                </div>
     ...
     absolute" href={github} onMouseEnter={() => { setShowGit(true) }}
                        ...
                        {demo ? <a className="flex items-center justify-center rounded-full font-semibold bg-dark text-light py-3 px-6 shadow-dark
     absolute left-28" target='_blank' href={demo} onMouseEnter={() => { setShowDemo(true) }}
